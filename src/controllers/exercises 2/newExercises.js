...
const addExercises = async (name, description, picture, type, muscleGroupId) => {
    let connection;
    try {
        ...
        //Comprueba si el ejercicio existe y genera error al no introducir todos los datos requeridos
...
const newExercise = async (req, res, next) => {
    try {
        ...
        const id = await addExercises(name, description, picture, type, muscleGroupId);
